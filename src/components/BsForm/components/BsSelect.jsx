...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const defaultFilterOption = (inputValue, option) => {
            return option?.label?.indexOf(inputValue) >= 0;
        };
        return () => {
            return <div class={['BsSelect', styles.width100]}>
        <dynamicSelect loading={optionsLoading.value} class={['select', styles.width100]} model-value={props.modelValue} placeholder={props.config.placeholder || `请选择${props.config?.label ?? ''}`} disabled={!!props.config.disabled} 
            ...
            /** ant-design-vue 特有属性-start */
            /** ele 特有属性-start */
            remote={props.config.remote === true} remote-method={props.config.remote === true ? remoteMethod : undefined} collapse-tags={props.config.collapseTags !== false} collapse-tags-tooltip={props.config.collapseTagsTooltip !== false} multiple-limit={props.config.multipleLimit ? props.config.multipleLimit : 0} reserveKeyword={props.config.reserveKeyword === true} 
            /** ele 特有属性-end */
            {...props.config.nativeProps} onChange={updateValue}>
          {options.value && Array.isArray(options.value) && options.value.map((item) => {
                    return <dynamicSelectOption key={item.value} label={item.label} value={item.value} {...props.config.nativeProps} v-slots={props.config.format ? {
                            default: () => {
...
//# sourceMappingURL=BsSelect.jsx.map
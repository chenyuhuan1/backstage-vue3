    ...
    setup(props) {
        ...
        return () => {
            const buttonDom = (button) => {
                return <dynamicButton type={button.type ?? 'primary'} size={button.size ?? 'small'} disabled={button.disabled} loading={loading.value} {...button.nativeProps} onClick={async () => {
                        loading.value = true;
          ...
          {props.buttons.map((button) => {
                    ...
                    if (button.confirmConfig && !button.disabled) {
                        return <dynamicPopconfirm title={button?.confirmConfig?.title ?? '标题'} 
                        ...
                        {...button?.confirmConfig?.nativeProps} onConfirm={() => {

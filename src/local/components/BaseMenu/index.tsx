 ...
 * @LastEditTime: 2023-09-01 17:06:50
 * @LastEditors: 陈宇环
 * @Description: 页面菜单组件
 */
  ...
  setup() {
    ...
    const recursion = (menus: any[]) => {
      return menus.map((menu:any) => {
        ...
        return (<>
          {(!menu.children && menu?.meta?.hide !== true) && <el-menu-item index={`/${menu.path}`} class={`/${menu.path}`} v-slots={{
            title: () => {
      ...
      return <div class={[styles.BaseMenu]}>
        ...
        >
          <a href='javascript:' onClick={() => {
            goApi()
          }} style={{ display: 'block', textAlign: 'center', padding: '10px 0' }}>API文档地址</a>
          {
